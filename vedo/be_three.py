...
def to_surf_mesh(actor, surf, mapper, prop, add_attr=None):
    ...
    * MeshBasicMaterial when lighting is disabled.
    ...
    if texture is not None:
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", message="Given trait value dtype")
            tjs_texture = tjs.DataTexture(
                data=data, format="RGBFormat", type="UnsignedByteType"
            )
...
def actor_to_mesh(actor, focal_point):
    ...
    if rep_type == "Surface" and has_faces:
        ...
        if has_faces:
            ...
            mesh = to_edge_mesh(
                dataset, mapper, prop, use_edge_coloring=False, use_lines=True
            )
            meshes.append(mesh)
...
def convert_renderer(pv_renderer):
    ...
    scene = tjs.Scene(
        children=children, background=pv_renderer.background_color.hex_rgb
    )
