...
class Viewer:
    ...
    """
    def __init__(self,box_type = "OpenPCDet",bg=(255, 255, 255), window_size=(800, 600), pos=(50,50)):
        self.objects_color_map = generate_objects_color_map('rainbow')
        self.box_type = box_type
        self.vi = Plotter(bg=bg, size=window_size, pos=pos)
        self.label_editor = None  # Keep a single instance of LabelEditor
        self.set_lights()
    ...
    def show_3D(self):
        ...
        if self.first_show:
            ...
            self.vi.show(self.actors+self.actors_without_del, resetcam=False,  camera={'pos': (-25, 0, 7), 'focalPoint': (5, 0, 2), 'viewup': (0, 0, 1)}, interactive=False)# added interactive=False for non-blocking reproduction
    ...
    def show_3D_BEV(self):
        """
        show objects in 3D scenes, before show_3D, you should add some objects into the current scenes
        :param bg_color: (tuple(3,) or list(3,) or str), background color of 3D scene
        :return:
        """
        vedo.plotter_instance = self.vi
    
        if self.first_show:
            #camera settings for an elevated frontal view: camera={'pos': (-25, 0, 7), 'focalPoint': (5, 0, 2), 'viewup': (0, 0, 1)}
            #camera settings camera={'pos': (35,0,80), 'focalPoint': (35,0,0 ),'viewup': (1, 0, 0),'parallelScale': 30, 'parallelProjection': True}, interactive=False, title="Bird's Eye View")# added interactive=False for non-blocking reproduction  
    ...
    def show_3D_right_lateral(self):
        """
        show objects in 3D scenes, before show_3D, you should add some objects into the current scenes
        :param bg_color: (tuple(3,) or list(3,) or str), background color of 3D scene
        :return:
        ...
        vedo.plotter_instance = self.vi
        if self.first_show:
            self.vi.show(self.actors+self.actors_without_del, resetcam=False,  camera={'pos': (42,-75,0), 'focalPoint': (42,0,0 ),'viewup': (0, 0, 1), 'parallelScale': 13, 'parallelProjection': True}, interactive=False, title="Lateral right view")# added interactive=False for non-blocking reproduction  
    ...
    def show_3D_left_lateral(self):
        ...
        vedo.plotter_instance = self.vi
        if self.first_show:
            self.vi.show(self.actors+self.actors_without_del, resetcam=False,  camera={'pos': (42,75,0), 'focalPoint': (42,0,0 ),'viewup': (0, 0, 1), 'parallelScale': 13, 'parallelProjection': True}, interactive=False, title="Lateral left view")# added interactive=False for non-blocking reproduction  
    ...
    def show_2D(self,box_color = (255,0,0),show_box_info=False,show_ids=True,points_colors=(0,0,255), show_box_heading=True):
        ...
        #cv2.resizeWindow('Front image', 900, 300)
        #cv2.moveWindow('Front image', 800, 700)
        resized_image = cv2.resize(self.image, (900, 300))
        #cv2.imshow('Front image',resized_image)
        #cv2.imshow('Front image',self.image)
        #cv2.waitKey(10)
