...
class KittiDetectionDataset:
    ...
    def __getitem__(self, item):
        ...
        #name = str(item).zfill(6) #names generated automatically with 6 digits and starts from 0, but the folder not always contains ordered files
        name = self.all_ids[item].split('.')[0]  # Use the actual filenames
        ...
        labelsOG=copy.copy(labels)
         # If the model didnt detect anything, label will be empty
        if len(labels) == 0 or len(label_names) == 0:
            print(f"Warning: No detections in file {label_path}")
            #print(f"these is the label_path just before the return: {label_path}")
            return P2, V2C, points, image, np.empty((0, 7)), [], labelsOG, name, label_path 
        
        labels[:,3:6] = cam_to_velo(labels[:,3:6],V2C)[:,:3]

        #print(f"these is the label_path just before the return: {label_path}")
        return P2,V2C,points,image,labels,label_names, labelsOG, name, label_path
